using System;
using System.Diagnostics.CodeAnalysis;
using System.Security.Cryptography;
using System.Text;

namespace SnowLib.Networking
{
    public class ClientPacket : Packet
    {
        private readonly MD5 md5 = MD5.Create();

        public ClientPacket(byte opcode)
        {
            Signature = 0xAA;
            bodyData = Array.Empty<byte>();
            Opcode = opcode;
        }

        public ClientPacket(byte[] rawData)
        {
            Signature = rawData[0];
            Length = (ushort) (rawData[1] * 256 + rawData[2]);
            Opcode = rawData[3];

            if (ShouldEncrypt)
            {
                Ordinal = rawData[4];
                bodyData = new byte[rawData.Length - 5];
                Array.Copy(rawData, 5, bodyData, 0, bodyData.Length);
            }
            else
            {
                bodyData = new byte[rawData.Length - 4];
                Array.Copy(rawData, 4, bodyData, 0, bodyData.Length);
            }
        }

        public sealed override bool ShouldEncrypt =>
            Opcode != 0x00 && Opcode != 0x0B && Opcode != 0x10 && Opcode != 0x57 && Opcode != 0x62;

        public override bool UseDefaultKey =>
            Opcode == 0x02 ||
            Opcode == 0x03 ||
            Opcode == 0x04 ||
            Opcode == 0x0B ||
            Opcode == 0x26 ||
            Opcode == 0x2D ||
            Opcode == 0x3A ||
            Opcode == 0x42 ||
            Opcode == 0x43 ||
            Opcode == 0x4B ||
            Opcode == 0x57 ||
            Opcode == 0x62 ||
            Opcode == 0x68 ||
            Opcode == 0x71 ||
            Opcode == 0x73 ||
            Opcode == 0x7B;

        public virtual void Decrypt(Client client)
        {
            var length = bodyData.Length - 7;

            var bRand = (ushort) (((bodyData[length + 6] << 8) | bodyData[length + 4]) ^ 0x7470);
            var sRand = (byte) (bodyData[length + 5] ^ 0x23);

            var key = UseDefaultKey ? client.Key : client.GenerateKey(bRand, sRand);

            for (var i = 0; i < length; i++)
            {
                bodyData[i] ^= key[i % key.Length];
                bodyData[i] ^= SaltTable[client.Seed][i / key.Length % SaltTable[client.Seed].Length];

                if (i / key.Length % SaltTable[client.Seed].Length != Ordinal)
                    bodyData[i] ^= SaltTable[client.Seed][Ordinal];
            }
        }

        public void DecryptDialog()
        {
            var xPrime = (byte) (bodyData[0] - 0x2D);
            var x = (byte) (bodyData[1] ^ xPrime);
            var y = (byte) (x + 0x72);
            var z = (byte) (x + 0x28);

            bodyData[2] ^= y;
            bodyData[3] ^= (byte) ((y + 1) % 256);

            var length = (bodyData[2] << 8) | bodyData[3];

            for (var i = 0; i < length; i++) bodyData[4 + i] ^= (byte) ((z + i) % 256);
        }

        public virtual void Encrypt(Client client)
        {
            var length = bodyData.Length - 7;

            var rand = new Random();
            var bRand = (ushort) (rand.Next(65277) + 256);
            var sRand = (byte) (rand.Next(155) + 100);

            var key = UseDefaultKey ? client.Key : client.GenerateKey(bRand, sRand);

            for (var i = 0; i < length; i++)
            {
                bodyData[i] ^= key[i % key.Length];
                bodyData[i] ^= SaltTable[client.Seed][i / key.Length % SaltTable[client.Seed].Length];
                if (i / key.Length % SaltTable[client.Seed].Length != Ordinal)
                    bodyData[i] ^= SaltTable[client.Seed][Ordinal];
            }

            var bytesToHash = new byte[length + 2];
            bytesToHash[0] = Opcode;
            bytesToHash[1] = Ordinal;
            Buffer.BlockCopy(bodyData, 0, bytesToHash, 2, length);

            var hash = md5.ComputeHash(bytesToHash);

            bodyData[length] = hash[13];
            bodyData[length + 1] = hash[3];
            bodyData[length + 2] = hash[11];
            bodyData[length + 3] = hash[7];
            bodyData[length + 4] = (byte) ((bRand % 256) ^ 0x70);
            bodyData[length + 5] = (byte) (sRand ^ 0x23);
            bodyData[length + 6] = (byte) (((bRand >> 8) % 256) ^ 0x74);
        }

        public void EncryptDialog()
        {
            var length = (bodyData[2] << 8) | bodyData[3];
            var xPrime = (byte) (bodyData[0] - 0x2D);
            var x = (byte) (bodyData[1] ^ xPrime);
            var y = (byte) (x + 0x72);
            var z = (byte) (x + 0x28);

            bodyData[2] ^= y;
            bodyData[3] ^= (byte) ((y + 1) % 256);

            for (var i = 0; i < length; i++) bodyData[4 + i] ^= (byte) ((z + i) % 256);
        }

        public void GenerateDialogHeader()
        {
            ushort crc = 0;

            for (var i = 0; i < bodyData.Length - 6; i++)
                crc = (ushort) (bodyData[6 + i] ^ (ushort) (crc << 8) ^ DialogCrcTable[crc >> 8]);

            var rand = new Random();
            bodyData[0] = (byte) rand.Next();
            bodyData[1] = (byte) rand.Next();
            bodyData[2] = (byte) ((bodyData.Length - 4) / 256);
            bodyData[3] = (byte) ((bodyData.Length - 4) % 256);
            bodyData[4] = (byte) (crc / 256);
            bodyData[5] = (byte) (crc % 256);
        }
    }

    public abstract class Packet
    {
        public static byte[][] SaltTable =
        {
            #region Seed 00

            new byte[]
            {
                
            },

            #endregion Seed 00

            #region Seed 01

            new byte[]
            {
                
            },

            #endregion Seed 01

            #region Seed 02

            new byte[]
            {
                
            },

            #endregion Seed 02

            #region Seed 03

            new byte[]
            {
                
            },

            #endregion Seed 03

            #region Seed 04

            new byte[]
            {
                
            },

            #endregion Seed 04

            #region Seed 05

            new byte[]
            {
                
            },

            #endregion Seed 05

            #region Seed 06

            new byte[]
            {
                
            },

            #endregion Seed 06

            #region Seed 07

            new byte[]
            {
                
            },

            #endregion Seed 07

            #region Seed 08

            new byte[]
            {
                
            },

            #endregion Seed 08

            #region Seed 09

            new byte[]
            {
                
            }

            #endregion Seed 09
        };

        // ReSharper disable once InconsistentNaming
        public byte[] bodyData;

        protected static ushort[] DialogCrcTable = new ushort[]
        {
            0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50A5, 0x60C6, 0x70E7,
            0x8108, 0x9129, 0xA14A, 0xB16B, 0xC18C, 0xD1AD, 0xE1CE, 0xF1EF,
            0x1231, 0x0210, 0x3273, 0x2252, 0x52B5, 0x4294, 0x72F7, 0x62D6,
            0x9339, 0x8318, 0xB37B, 0xA35A, 0xD3BD, 0xC39C, 0xF3FF, 0xE3DE,
            0x2462, 0x3443, 0x0420, 0x1401, 0x64E6, 0x74C7, 0x44A4, 0x5485,
            0xA56A, 0xB54B, 0x8528, 0x9509, 0xE5EE, 0xF5CF, 0xC5AC, 0xD58D,
            0x3653, 0x2672, 0x1611, 0x0630, 0x76D7, 0x66F6, 0x5695, 0x46B4,
            0xB75B, 0xA77A, 0x9719, 0x8738, 0xF7DF, 0xE7FE, 0xD79D, 0xC7BC,
            0x48C4, 0x58E5, 0x6886, 0x78A7, 0x0840, 0x1861, 0x2802, 0x3823,
            0xC9CC, 0xD9ED, 0xE98E, 0xF9AF, 0x8948, 0x9969, 0xA90A, 0xB92B,
            0x5AF5, 0x4AD4, 0x7AB7, 0x6A96, 0x1A71, 0x0A50, 0x3A33, 0x2A12,
            0xDBFD, 0xCBDC, 0xFBBF, 0xEB9E, 0x9B79, 0x8B58, 0xBB3B, 0xAB1A,
            0x6CA6, 0x7C87, 0x4CE4, 0x5CC5, 0x2C22, 0x3C03, 0x0C60, 0x1C41,
            0xEDAE, 0xFD8F, 0xCDEC, 0xDDCD, 0xAD2A, 0xBD0B, 0x8D68, 0x9D49,
            0x7E97, 0x6EB6, 0x5ED5, 0x4EF4, 0x3E13, 0x2E32, 0x1E51, 0x0E70,
            0xFF9F, 0xEFBE, 0xDFDD, 0xCFFC, 0xBF1B, 0xAF3A, 0x9F59, 0x8F78,
            0x9188, 0x81A9, 0xB1CA, 0xA1EB, 0xD10C, 0xC12D, 0xF14E, 0xE16F,
            0x1080, 0x00A1, 0x30C2, 0x20E3, 0x5004, 0x4025, 0x7046, 0x6067,
            0x83B9, 0x9398, 0xA3FB, 0xB3DA, 0xC33D, 0xD31C, 0xE37F, 0xF35E,
            0x02B1, 0x1290, 0x22F3, 0x32D2, 0x4235, 0x5214, 0x6277, 0x7256,
            0xB5EA, 0xA5CB, 0x95A8, 0x8589, 0xF56E, 0xE54F, 0xD52C, 0xC50D,
            0x34E2, 0x24C3, 0x14A0, 0x0481, 0x7466, 0x6447, 0x5424, 0x4405,
            0xA7DB, 0xB7FA, 0x8799, 0x97B8, 0xE75F, 0xF77E, 0xC71D, 0xD73C,
            0x26D3, 0x36F2, 0x0691, 0x16B0, 0x6657, 0x7676, 0x4615, 0x5634,
            0xD94C, 0xC96D, 0xF90E, 0xE92F, 0x99C8, 0x89E9, 0xB98A, 0xA9AB,
            0x5844, 0x4865, 0x7806, 0x6827, 0x18C0, 0x08E1, 0x3882, 0x28A3,
            0xCB7D, 0xDB5C, 0xEB3F, 0xFB1E, 0x8BF9, 0x9BD8, 0xABBB, 0xBB9A,
            0x4A75, 0x5A54, 0x6A37, 0x7A16, 0x0AF1, 0x1AD0, 0x2AB3, 0x3A92,
            0xFD2E, 0xED0F, 0xDD6C, 0xCD4D, 0xBDAA, 0xAD8B, 0x9DE8, 0x8DC9,
            0x7C26, 0x6C07, 0x5C64, 0x4C45, 0x3CA2, 0x2C83, 0x1CE0, 0x0CC1,
            0xEF1F, 0xFF3E, 0xCF5D, 0xDF7C, 0xAF9B, 0xBFBA, 0x8FD9, 0x9FF8,
            0x6E17, 0x7E36, 0x4E55, 0x5E74, 0x2E93, 0x3EB2, 0x0ED1, 0x1EF0
        };

        private readonly Encoding encoding = Encoding.GetEncoding(949);
        private int position;

        public byte[] Header
        {
            get
            {
                var header = new byte[ShouldEncrypt ? 5 : 4];
                header[0] = Signature;
                header[1] = (byte) (Length / 256);
                header[2] = (byte) (Length % 256);
                header[3] = Opcode;
                if (ShouldEncrypt) header[4] = Ordinal;
                return header;
            }
        }

        public ushort Length { get; set; }
        public byte Opcode { get; set; }
        public byte Ordinal { get; set; }

        public int Position
        {
            get => position;
            set => position = value;
        }

        public abstract bool ShouldEncrypt { get; }
        public byte Signature { get; set; }
        public abstract bool UseDefaultKey { get; }

        public byte[] BodyData
        {
            get => bodyData;
            set => bodyData = value;
        }

        public byte[] Read(int length)
        {
            if (position + (length - 1) >= bodyData.Length)
                throw new IndexOutOfRangeException();

            var buffer = new byte[length];
            Buffer.BlockCopy(bodyData, position, buffer, 0, length);
            position += length;
            return buffer;
        }

        public bool ReadBoolean()
        {
            if (position < bodyData.Length) return bodyData[position++] != 0;
            throw new IndexOutOfRangeException();
        }

        public byte ReadByte()
        {
            if (position < bodyData.Length) return bodyData[position++];
            throw new IndexOutOfRangeException();
        }

        public short ReadInt16()
        {
            if (position + 1 < bodyData.Length) return (short) ((bodyData[position++] << 8) | bodyData[position++]);
            throw new IndexOutOfRangeException();
        }

        public int ReadInt32()
        {
            if (position + 3 < bodyData.Length)
                return (bodyData[position++] << 24) |
                       (bodyData[position++] << 16) |
                       (bodyData[position++] << 8) |
                       bodyData[position++];
            throw new IndexOutOfRangeException();
        }

        [SuppressMessage("ReSharper", "ShiftExpressionRightOperandNotEqualRealCount")]
        [SuppressMessage("ReSharper", "ShiftExpressionRealShiftCountIsZero")]
        public long ReadInt64()
        {
            if (position + 7 < bodyData.Length)
                return (bodyData[position++] << 56) |
                       (bodyData[position++] << 48) |
                       (bodyData[position++] << 40) |
                       (bodyData[position++] << 32) |
                       (bodyData[position++] << 24) |
                       (bodyData[position++] << 16) |
                       (bodyData[position++] << 8) |
                       bodyData[position++];
            throw new IndexOutOfRangeException();
        }

        public sbyte ReadSByte()
        {
            if (position < bodyData.Length) return (sbyte) bodyData[position++];
            throw new IndexOutOfRangeException();
        }

        public string ReadString()
        {
            return ReadString(bodyData.Length - position).Trim('\0');
        }

        public string ReadString(int length)
        {
            if (position + (length - 1) >= bodyData.Length)
                throw new IndexOutOfRangeException();

            var buffer = new byte[length];
            Buffer.BlockCopy(bodyData, position, buffer, 0, length);
            position += length;
            return encoding.GetString(buffer);
        }

        public string ReadString16()
        {
            var length = ReadUInt16();
            return ReadString(length);
        }

        public string ReadString8()
        {
            var length = ReadByte();
            return ReadString(length);
        }

        public ushort ReadUInt16()
        {
            if (position + 1 < bodyData.Length) return (ushort) ((bodyData[position++] << 8) | bodyData[position++]);
            throw new IndexOutOfRangeException();
        }

        public uint ReadUInt32()
        {
            if (position + 3 < bodyData.Length)
                return (uint) ((bodyData[position++] << 24) |
                               (bodyData[position++] << 16) |
                               (bodyData[position++] << 8) |
                               bodyData[position++]);
            throw new IndexOutOfRangeException();
        }

        [SuppressMessage("ReSharper", "ShiftExpressionRightOperandNotEqualRealCount")]
        [SuppressMessage("ReSharper", "ShiftExpressionRealShiftCountIsZero")]
        public ulong ReadUInt64()
        {
            if (position + 7 < bodyData.Length)
                return (ulong) ((bodyData[position++] << 56) |
                                (bodyData[position++] << 48) |
                                (bodyData[position++] << 40) |
                                (bodyData[position++] << 32) |
                                (bodyData[position++] << 24) |
                                (bodyData[position++] << 16) |
                                (bodyData[position++] << 8) |
                                bodyData[position++]);
            throw new IndexOutOfRangeException();
        }

        public override string ToString()
        {
            return $"{Opcode:X2} {BitConverter.ToString(bodyData).Replace('-', ' ')}";
        }

        public void Write(byte[] value)
        {
            foreach (var t in value) WriteByte(t);
        }

        public void WriteByte(byte value)
        {
            var length = bodyData.Length;
            Array.Resize(ref bodyData, length + 1);
            bodyData[length] = value;
        }

        public void WriteByte(bool value)
        {
            var length = bodyData.Length;
            Array.Resize(ref bodyData, length + 1);
            bodyData[length] = (byte) (value ? 1 : 0);
        }

        public void WriteInt16(short value)
        {
            var data = BitConverter.GetBytes(value);
            if (BitConverter.IsLittleEndian) Array.Reverse(data);
            Write(data);
        }

        public void WriteInt32(int value)
        {
            var data = BitConverter.GetBytes(value);
            if (BitConverter.IsLittleEndian) Array.Reverse(data);
            Write(data);
        }

        public void WriteInt64(long value)
        {
            var data = BitConverter.GetBytes(value);
            if (BitConverter.IsLittleEndian) Array.Reverse(data);
            Write(data);
        }

        public void WriteSByte(sbyte value)
        {
            var length = bodyData.Length;
            Array.Resize(ref bodyData, length + 1);
            bodyData[length] = (byte) value;
        }

        public void WriteString(string value)
        {
            var buffer = Encoding.GetEncoding(949).GetBytes(value);
            Write(buffer);
        }

        public void WriteString16(string value)
        {
            var buffer = Encoding.GetEncoding(949).GetBytes(value);
            WriteUInt16((ushort) buffer.Length);
            Write(buffer);
        }

        public void WriteString16(string format, params object[] args)
        {
            WriteString16(string.Format(format, args));
        }

        public void WriteString8(string value)
        {
            var buffer = Encoding.GetEncoding(949).GetBytes(value);
            WriteByte((byte) buffer.Length);
            Write(buffer);
        }

        public void WriteString8(string format, params object[] args)
        {
            WriteString8(string.Format(format, args));
        }

        public void WriteUInt16(ushort value)
        {
            var data = BitConverter.GetBytes(value);
            if (BitConverter.IsLittleEndian) Array.Reverse(data);
            Write(data);
        }

        public void WriteUInt32(uint value)
        {
            var data = BitConverter.GetBytes(value);
            if (BitConverter.IsLittleEndian) Array.Reverse(data);
            Write(data);
        }

        public void WriteUInt64(ulong value)
        {
            var data = BitConverter.GetBytes(value);
            if (BitConverter.IsLittleEndian) Array.Reverse(data);
            Write(data);
        }
    }

    public class ServerPacket : Packet
    {
        public ServerPacket(byte opcode)
        {
            Signature = 0xAA;
            bodyData = Array.Empty<byte>();
            Opcode = opcode;
        }

        public ServerPacket(byte[] rawData)
        {
            Signature = rawData[0];
            Length = (ushort) (rawData[1] * 256 + rawData[2]);
            Opcode = rawData[3];

            if (ShouldEncrypt)
            {
                Ordinal = rawData[4];
                bodyData = new byte[rawData.Length - 5];
                Array.Copy(rawData, 5, bodyData, 0, bodyData.Length);
            }
            else
            {
                bodyData = new byte[rawData.Length - 4];
                Array.Copy(rawData, 4, bodyData, 0, bodyData.Length);
            }
        }

        public sealed override bool ShouldEncrypt =>
            Opcode != 0x00 && Opcode != 0x03 && Opcode != 0x6F && Opcode != 0x7E;

        public override bool UseDefaultKey =>
            Opcode == 0x01 ||
            Opcode == 0x02 ||
            Opcode == 0x0A ||
            Opcode == 0x56 ||
            Opcode == 0x60 ||
            Opcode == 0x62 ||
            Opcode == 0x66 ||
            Opcode == 0x6F;

        public virtual void Decrypt(Client client)
        {
            var length = bodyData.Length - 3;

            var bRand = (ushort) (((bodyData[length + 2] << 8) | bodyData[length]) ^ 0x6474);
            var sRand = (byte) (bodyData[length + 1] ^ 0x24);

            var key = UseDefaultKey ? client.Key : client.GenerateKey(bRand, sRand);

            for (var i = 0; i < length; i++)
            {
                if (i % key.Length >= 0 && key.Length > i % key.Length)
                    bodyData[i] ^= key[i % key.Length];

                if (i / key.Length % SaltTable[client.Seed].Length >= 0 && SaltTable[client.Seed].Length >
                    i / key.Length % SaltTable[client.Seed].Length)
                    bodyData[i] ^= SaltTable[client.Seed][i / key.Length % SaltTable[client.Seed].Length];

                if (i / key.Length % SaltTable[client.Seed].Length != Ordinal)
                    bodyData[i] ^= SaltTable[client.Seed][Ordinal];
            }
        }

        public virtual void Encrypt(Client client)
        {
            var length = bodyData.Length - 3;

            var rand = new Random();
            var bRand = (ushort) (rand.Next() % 65277 + 256);
            var sRand = (byte) (rand.Next() % 155 + 100);

            var key = UseDefaultKey ? client.Key : client.GenerateKey(bRand, sRand);

            for (var i = 0; i < length; i++)
            {
                if (i % key.Length >= 0 && key.Length > i % key.Length)
                    bodyData[i] ^= key[i % key.Length];

                if (i / key.Length % SaltTable[client.Seed].Length >= 0 && SaltTable[client.Seed].Length >
                    i / key.Length % SaltTable[client.Seed].Length)
                    bodyData[i] ^= SaltTable[client.Seed][i / key.Length % SaltTable[client.Seed].Length];

                if (i / key.Length % SaltTable[client.Seed].Length != Ordinal)
                    bodyData[i] ^= SaltTable[client.Seed][Ordinal];
            }

            bodyData[length] = (byte) ((bRand % 256) ^ 0x74);
            bodyData[length + 1] = (byte) (sRand ^ 0x24);
            bodyData[length + 2] = (byte) (((bRand >> 8) % 256) ^ 0x64);
        }
    }
}